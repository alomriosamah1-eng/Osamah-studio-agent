...
أُغلقت بعد الدراسة شريحتا External Accounts metadata-only وStorage Settings read-only، ثم Self-development Candidate Review وMemory Consolidation وSQLite memory persistence bounded وlocal lexical retrieval bounded. أثبت runtime أن FTS5 غير متوفر؛ الشريحة التقنية التالية هي relational links وscope/visibility filtering أو FTS adapter اختياري بعد مراجعة مستقلة، وتبقى OAuth/MCP/Playwright/tunnels خلف gates مستقلة، ولا تُضاف embeddings أو provider sharing أو skill activation تلقائيًا.
