...
| حالة الشجرة | SQLite memory persistence feature/docs-close `48daaf1f83bbc4cc7f01ff2a4e873c5e1a9a31ad`/`939a7e07078a0a14d743f3e2e5d23435d2906150` وlocal lexical retrieval feature/docs-close `57376c3363b7d3b0670f7395bc61ea5e2613738b`/`76490ec29c5280c2d062c8f6f1eeeb056bd4fcb1` وvisibility-filtered retrieval feature/docs-close `cf3227331384ab2f5b5b126fb0cf381c6ea8b8f1`/`da38ca40fa1d0b247a37ee785fce1dfd028ad9e7` متحققون local==remote؛ relational MemoryEntry links feature/docs-close `b4658e22690cc1b69d43fc1d25c5cffa6f72844d`/`96fbf39eb1666838b64537a1126f2565d7cfd163` متحققان local==remote؛ agent scope feature `909b67c2e8c2ffa764b5139965252f54cf710601` دُفع وتحقق local==remote؛ docs-close مستقل قيد الإعداد. يسبقها Self-development feature `d35084225c3444608653e87b9a46c76e11c28d06` وdocs-close مستقل متحقق، وStorage Settings feature `fa6486a3a14784d03dd00a868fb5e98c4ff4d2eb` وdocs-close مستقل متحقق، وExternal Accounts feature `1b7d9df2d997776fe5453c7d8ea7c4e4c8fc3a4b` وdocs-close مستقل متحقق. Memory Review مغلقة عند feature `4f1709cd927b77627c4532ec396f572f3bbedb2c` وdocs-close مستقل متحقق. Audit documentation feature/docs-close متحققان عند `7ede715dfbd8f9e4d030848573b11e94790ba580` و`f0c3d86c329f45335cd22cc1465ba673435fea87`. Agent Catalog feature/docs-close متحققان عند `56eebb97267c1e5979ea2e53c9f3fa9ec988d192` و`bc6c9ea3bf430e9586307390e0843fa5fe7c0b5d`. ReportDocument feature/docs-close متحققان عند `24144c4f2495354b5d2d8a5a880192dc251173ff` و`56db328b51b42b64ea59830979df3e262d126237`. Settings feature `d9a746c7deecb38488a5f632599be16c48c8e8cb` دُفع وتحقق local==remote؛ docs-close توثيقي مستقل لهذه التسليمة |
| الإصدار المحلي | `0.6.0`؛ لا يوجد bump إصدار release في هذه الشريحة |
| آخر فحص مكتمل | full gate ناجح في 2026-08-22؛ `pnpm check` بـ`208/208`، و`pnpm build` و`pnpm desktop:smoke` و`pnpm performance:smoke` وSQLite/JSON/Node syntax/diff/secret gates ناجحة؛ log في `research/memory-agent-scope-full-gate-output-2026-08-22.txt` |
| schema الحالي | migrations `001` ثم `002` ثم `003` ثم `004` ثم `005_memory_persistence.sql` ثم `006_memory_entry_links.sql`، schema version `006`، 14 جدولًا و30 فهرسًا |
...
| External Accounts | `docs/87-external-accounts-metadata-only.md`؛ `IMPLEMENTED / METADATA-ONLY`، feature `1b7d9df2d997776fe5453c7d8ea7c4e4c8fc3a4b` وdocs-close `903ad33bedead375a29ed855a339e77ad303113b` متحققان local==remote |
| Storage Settings | `docs/88-storage-settings-read-only.md`؛ `IMPLEMENTED / READ-ONLY SNAPSHOT`، لا نقل أو حذف أو backup/restore من UI، feature/docs-close متحققان |
| Self-development | `docs/89-self-development-candidate-review.md`؛ `IMPLEMENTED / REVIEW-ONLY BOUNDED`، المرشح يبدأ `review_required`، والتفعيل صريح للمرشح النظيف فقط، feature `d35084225c3444608653e87b9a46c76e11c28d06` متحقق local==remote، docs-close مستقل متحقق |
| Memory Consolidation | `docs/90-memory-consolidation-bounded.md`؛ `IMPLEMENTED / BOUNDED REVIEW`، لا embeddings أو provider sharing أو mutation للمصدر، feature `f30c257faafb1159f7c34344504312b3eef16119` متحقق local==remote، docs-close قيد الإعداد |
| Memory Persistence | `docs/91-memory-persistence-sqlite.md`؛ `IMPLEMENTED / OPTIONAL SQLITE`، restart round-trip وredaction وfail-closed hydration، دون FTS أو embeddings أو vector services أو provider sharing أو automatic consolidation، feature/docs-close `48daaf1f83bbc4cc7f01ff2a4e873c5e1a9a31ad`/`939a7e07078a0a14d743f3e2e5d23435d2906150` متحققان local==remote |
| Local Lexical Retrieval | `docs/92-memory-local-retrieval-bounded.md`؛ `IMPLEMENTED / LEXICAL BOUNDED`، Arabic/English normalization وall-token matching وترتيب title/tag/content، دون FTS5 أو semantic/vector/provider calls، feature/docs-close `57376c3363b7d3b0670f7395bc61ea5e2613738b`/`76490ec29c5280c2d062c8f6f1eeeb056bd4fcb1` متحققان local==remote |
| Visibility-filtered Retrieval | `docs/92-memory-local-retrieval-bounded.md`؛ `IMPLEMENTED / EXACT VISIBILITY FILTER`، المرشح private/workspace/project فقط، typed validation وfail-closed دون توسيع الوصول، feature/docs-close `cf3227331384ab2f5b5b126fb0cf381c6ea8b8f1`/`da38ca40fa1d0b247a37ee785fce1dfd028ad9e7` متحققان local==remote |
| Relational Memory Links | `docs/93-memory-relational-links-bounded.md`؛ `IMPLEMENTED / BOUNDED DIRECTED LINKS`، migration 006 وrelations ثابتة وtarget/visibility validation وfail-closed hydration، feature/docs-close `b4658e22690cc1b69d43fc1d25c5cffa6f72844d`/`96fbf39eb1666838b64537a1126f2565d7cfd163` متحققان local==remote |
| Agent Scope Filtering | `docs/94-memory-agent-scope-bounded.md`؛ `IMPLEMENTED / BOUNDED POLICY FILTER`، AgentCatalog يفرض visibility/retention/providerAccess قبل lexical scoring، unknown/unsafe agentId fail-closed، feature `909b67c2e8c2ffa764b5139965252f54cf710601` متحقق local==remote، docs-close قيد الإعداد |
...
| `python3 scripts/validate_sqlite_migration.py` | ناجح؛ migration count `6`، schema `006`، 14 جدولًا، 30 فهرسًا |
...
| desktop smoke | `DESKTOP_ROOT_PICKER_SMOKE=PASS` و`DESKTOP_SMOKE=PASS` و`DESKTOP_IPC_SMOKE=PASS`؛ task.preview وContent Plan وAsset Catalog/Creative Brief وArtifact Assembly وRender Policy وMemory Capture/Review وlocal retrieval no-mutation PASS |
...
دراسة Virtual Human مكتملة توثيقيًا فقط: `docs/84-virtual-human-research-and-comparison.md` للمقارنة، `docs/85-virtual-human-architecture-and-contracts.md` للعقود والحالات والأداء والخصوصية، و`docs/86-virtual-human-licensing-roadmap-and-decisions.md` للتراخيص وroadmap 0–11. commit الدراسة `d47294ed613090bc913ee14d68669621febbccbb` وdocs-close `7ce4d830c4debdc9c65f7d5ad8729f62454dc99f` متحققان local==remote. لا يبدأ تنفيذ Avatar أو تثبيت حزم أو تغيير Electron/TypeScript/typed IPC قبل قرار مالك مستقل.

أُغلقت بعد الدراسة شريحتا External Accounts metadata-only وStorage Settings read-only، ثم Self-development Candidate Review وMemory Consolidation وSQLite memory persistence bounded وlocal lexical retrieval bounded، ثم visibility-filtered retrieval. أثبت runtime أن FTS5 غير متوفر؛ الشريحة التقنية التالية هي relational links وagent scope/permission filtering أو FTS adapter اختياري بعد مراجعة مستقلة، وتبقى OAuth/MCP/Playwright/tunnels خلف gates مستقلة، ولا تُضاف embeddings أو provider sharing أو skill activation تلقائيًا.
