...
| Memory Persistence | `docs/91-memory-persistence-sqlite.md`؛ `IMPLEMENTED / OPTIONAL SQLITE`، restart round-trip وredaction وfail-closed hydration، دون FTS أو embeddings أو vector services أو provider sharing أو automatic consolidation، feature `48daaf1f83bbc4cc7f01ff2a4e873c5e1a9a31ad` متحقق local==remote، docs-close قيد الإعداد |
...
أُغلقت بعد الدراسة شريحتا External Accounts metadata-only وStorage Settings read-only، ثم Self-development Candidate Review وMemory Consolidation وSQLite memory persistence bounded؛ الشريحة التقنية التالية هي تقييم local retrieval/FTS، وتبقى OAuth/MCP/Playwright/tunnels خلف gates مستقلة، ولا تُضاف embeddings أو provider sharing أو skill activation تلقائيًا.
