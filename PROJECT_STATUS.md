...
| آخر فحص مكتمل | `pnpm check` ناجح، `201/201` اختبارًا؛ `pnpm build` و`pnpm desktop:smoke` و`pnpm performance:smoke` وSQLite/JSON/diff/secret gates ناجحة، وMemory Consolidation IPC/Electron smoke ناجح في 2026-08-22 |
...
أُغلقت بعد الدراسة شريحتا External Accounts metadata-only وStorage Settings read-only؛ الشريحة التقنية التالية هي Self-development Candidate Review، وتبقى OAuth/MCP/Playwright/tunnels خلف gates مستقلة، ولا تُضاف persistence أو embeddings أو skill activation تلقائيًا.
