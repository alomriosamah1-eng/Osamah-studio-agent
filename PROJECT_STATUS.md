...
| آخر فحص مكتمل | `pnpm check` ناجح، `177/177` اختبارًا؛ `pnpm build` و`pnpm desktop:smoke` و`pnpm performance:smoke` وSQLite/JSON/diff/secret gates ناجحة، وReportDocument IPC/Electron smoke ناجح في 2026-08-22 |
...
بعد إغلاق ReportDocument، تبدأ الشريحة التقنية التالية `PreviewSharePort` أو `bounded local memory links` حسب قرار المالك؛ تبقى OAuth/MCP/Playwright/tunnels خلف gates مستقلة، ولا تُضاف Markdown/PDF export أو persistence أو embeddings تلقائيًا.
