...
| المرحلة | SQLite memory persistence وlocal lexical retrieval وvisibility-filtered retrieval وrelational MemoryEntry links وagent scope filtering وMarkdown export preview وdestination write الآمنة مكتملة؛ OpenCode SDK وHermes ACP SDK مدمجان فعليًا كـprovider adapters اختياريين خلف `ProviderGateway`؛ واجهة Osamah الموحدة محمية باختبار boundary؛ semantic memory قرارها موثق ومؤجل في `docs/96-semantic-memory-deferred-decision.md`؛ المسار التالي Monaco Editor surface integration review ثم render workers/format review؛ DeepSeek Harness قرار `deferred-not-selected` موثق في `docs/100-deepseek-harness-bridge-decision.md`؛ Virtual Human study موثقة فقط وdeferred |
| الحالة | Memory Consolidation bounded منفذة في `docs/90-memory-consolidation-bounded.md`؛ feature `f30c257faafb1159f7c34344504312b3eef16119` دُفع وتحقق local==remote؛ docs-close توثيقي مستقل قيد الإعداد. Self-development Candidate Review منفذة في `docs/89-self-development-candidate-review.md`؛ feature `d35084225c3444608653e87b9a46c76e11c28d06` وdocs-close مستقل متحققان. External Accounts metadata-only منفذة في `docs/87-external-accounts-metadata-only.md` وStorage Settings في `docs/88-storage-settings-read-only.md`. Virtual Human / AI Avatar docs `84–86` وresearch metadata مكتملة كـDOCUMENTED ONLY؛ commit الدراسة `d47294ed613090bc913ee14d68669621febbccbb` دُفع وتحقق local==remote؛ لا Avatar implementation أو packages/models/assets. Memory Review مغلقة عند feature `4f1709cd927b77627c4532ec396f572f3bbedb2c` وdocs-close `04e49b1590fe7df529636e8c721f5b57b86d439f`. Audit documentation feature/docs-close متحققان عند `7ede715dfbd8f9e4d030848573b11e94790ba580` و`f0c3d86c329f45335cd22cc1465ba673435fea87`. Agent Catalog feature/docs-close متحققان عند `56eebb97267c1e5979ea2e53c9f3fa9ec988d192` و`bc6c9ea3bf430e9586307390e0843fa5fe7c0b5d`. ReportDocument feature/docs-close متحققان عند `24144c4f2495354b5d2d8a5a880192dc251173ff` و`56db328b51b42b64ea59830979df3e262d126237`. Settings feature `d9a746c7deecb38488a5f632599be16c48c8e8cb` دُفع وتحقق local==remote؛ docs-close توثيقي مستقل لهذه التسليمة. SQLite memory persistence feature/docs-close `48daaf1f83bbc4cc7f01ff2a4e873c5e1a9a31ad`/`939a7e07078a0a14d743f3e2e5d23435d2906150` وlocal lexical retrieval feature/docs-close `57376c3363b7d3b0670f7395bc61ea5e2613738b`/`76490ec29c5280c2d062c8f6f1eeeb056bd4fcb1` متحققان؛ visibility-filtered retrieval feature/docs-close `cf3227331384ab2f5b5b126fb0cf381c6ea8b8f1`/`da38ca40fa1d0b247a37ee785fce1dfd028ad9e7` متحققان؛ relational MemoryEntry links feature/docs-close `b4658e22690cc1b69d43fc1d25c5cffa6f72844d`/`96fbf39eb1666838b64537a1126f2565d7cfd163` متحققان؛ agent scope feature/docs-close `909b67c2e8c2ffa764b5139965252f54cf710601`/`480d0caf9b706c529b2706eaa9ed2735e10dad1e` متحققان local==remote؛ Markdown export feature/docs-close `194730e0c5777f8ff50e184345fca1518492c49e`/`e2a38cc7b7be8451a653dbc08789048c8c23d886` متحققان local==remote؛ semantic memory deferred decision `docs/96-semantic-memory-deferred-decision.md` متحقق؛ Markdown destination feature `1de463fadd188373eef2bc418f6a6ab6f6d7fa2a` وdocs-close `82d920d0549b11ce8e357f082458fa8ff2caad9d` متحققان local==remote؛ OpenCode SDK feature `ce4fae4d68636cb4fbc34bbea70720ac5016e788` متحقق local==remote، docs-close في الإعداد |
...
| آخر فحص | full gate بعد Hermes ACP وunified UI boundary ناجح؛ `pnpm check` بـ`222/222`؛ build وdesktop/performance smoke وSQLite/JSON/Node syntax/diff/secret validation PASS؛ schema 006 و14 جدولًا و30 فهرسًا؛ log في `research/hermes-acp-integration-full-gate-output-2026-08-23.txt` |
| schema | migrations `001` ثم `002` ثم `003` ثم `004` ثم `005_memory_persistence.sql` ثم `006_memory_entry_links.sql`، schema version `006`؛ 14 جدولًا و30 فهرسًا؛ لا migration جديدة لإضافة upstream SDKs |
...
أضيفت `LocalHttpProviderAdapter` المشتركة و`OllamaProviderAdapter` و`LlamaCppProviderAdapter` مع loopback-only URLs وtimeout/cancellation وinput/output bounds وtyped HTTP/output errors. ثم أضيف `OpenCodeSdkProviderAdapter` و`HermesAcpProviderAdapter` كـupstream bridges opt-in؛ تسجيلها في composition اختياري ولا يجري health probe أو model/provider/worker loading عند startup. Hermes ACP يستخدم child process stdio، read-only workspace، ورفض mutation/terminal/permission في هذه الشريحة.
...
| `pnpm desktop:smoke` | ناجح؛ `DESKTOP_ROOT_PICKER_SMOKE=PASS` و`DESKTOP_IPC_SMOKE=PASS` وtask.preview وSource Registry وContent Plan وAsset Catalog/Creative Brief وArtifact Assembly وRender Policy وMemory Capture/Review وlocal retrieval وagent scope no-mutation PASS |
| composition SQLite | opt-in/restart/fallback/close lifecycle وmigration 006 وMemoryEntry links وagent scope restart/fail-closed PASS؛ Markdown preview no-write PASS |
...
لا توجد بعد React Native Web/Metro runtime فعلية، ولا Android doctor/ADB adapter، ولا iOS Xcode adapter، ولا تكاملات remote/EAS. لا ينبغي تشغيل native toolchains أو scripts من مشاريع الهاتف تلقائيًا. واجهة Osamah لا تستورد OpenCode/Hermes/DeepSeek UI؛ اختبار `unified-ui-boundary.test.ts` يحرس Preload وIPC وWorkspace prototype من upstream UI imports.
...
دراسة Virtual Human داخل Second Brain أُغلقت توثيقيًا فقط في docs `84–86`، مع roadmap 0–11 وLicense Matrix وstate/event design وprivacy/performance budget. لا يبدأ تنفيذها أو تثبيت حزم/نماذج أو تغيير Electron/TypeScript/typed IPC إلا بعد قرار مالك منفصل. أُغلقت local lexical retrieval والروابط وscope/visibility filtering؛ أثبت runtime أن FTS5 غير متوفر. أُضيف OpenCode SDK ثم Hermes ACP worker فعليًا خلف adapters، بينما DeepSeek Harness قرار مؤجل غير مختار لطبقة plugin/event spine ولا يضاف إلا عند capability gap وبوابات توافق مستقلة. المسار التالي Monaco Editor surface integration خلف `EditorSurfacePort`. تبقى embeddings/vector services وOAuth/MCP/Playwright/tunnels خلف gates مستقلة.
