...
| المرحلة | SQLite memory persistence وlocal lexical retrieval وvisibility-filtered retrieval وrelational MemoryEntry links وagent scope filtering وMarkdown export preview وdestination write الآمنة مكتملة؛ OpenCode SDK مدمج فعليًا كـprovider adapter اختياري في `src/infrastructure/opencode-sdk-provider.ts`؛ semantic memory قرارها موثق ومؤجل في `docs/96-semantic-memory-deferred-decision.md`؛ المسار التالي DeepSeek/Hermes harness bridge review ثم render workers/format review؛ Virtual Human study موثقة فقط وdeferred |
...
| آخر فحص | full gate بعد OpenCode SDK integration ناجح؛ `pnpm check` بـ`217/217`؛ build وdesktop/performance smoke وSQLite/JSON/Node syntax/diff/secret validation PASS؛ schema 006 و14 جدولًا و30 فهرسًا؛ log في `research/opencode-sdk-integration-full-gate-output-2026-08-23.txt` |
| schema | migrations `001` ثم `002` ثم `003` ثم `004` ثم `005_memory_persistence.sql` ثم `006_memory_entry_links.sql`، schema version `006`؛ 14 جدولًا و30 فهرسًا |
