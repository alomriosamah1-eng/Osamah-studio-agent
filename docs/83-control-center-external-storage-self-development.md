...
أُغلقت شريحة **إعدادات العرض العامة**: locale/theme/fontScale/density/reduceMotion. تلتها شريحة `External Accounts — Metadata-only` الموثقة في `docs/87-external-accounts-metadata-only.md`؛ تسجل provider/label/owner/scopes وresourceScope في الذاكرة فقط، وتبدأ `disconnected` مع `consent_required` و`unknown` verification، دون OAuth أو MCP أو persistence أو secrets أو network. ثم أُغلقت شريحة `Storage Settings — Read-only Snapshot` الموثقة في `docs/88-storage-settings-read-only.md`؛ تعرض backend/profile/schema/lock/backup/retention/quota دون تغيير أو نقل أو حذف أو backup/restore. ثم أُغلقت شريحة `Self-development Candidate Review وRule Overlay` الموثقة في `docs/89-self-development-candidate-review.md`؛ تستقبل instruction/strategy/plan/skill في `review_required`، وتعرض preview وتسمح بتفعيل أو أرشفة أو rollback صريح للمرشح النظيف فقط، دون تنفيذ المحتوى أو تعديل core policy أو الصلاحيات أو provider access. لا تُفعل OAuth أو MCP أو skill execution تلقائيًا، ولا تُضاف قاعدة معرفية دائمة قبل تصميم migration والخصوصية والـrollback.
