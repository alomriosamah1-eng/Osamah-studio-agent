...
الاختيار الأساسي هو **Modular Desktop Monolith مع Process Isolation**، وليس microservices في النسخة المحلية الأولى. يتكون النظام من Desktop Shell، Renderer UI، Typed Preload/IPC، Application Core، Agent Runtime، Provider Gateway، Policy Engine، Data Layer، وWorkers. هذا الفصل يدمج الأجزاء المناسبة من upstream بدل إعادة بنائها: OpenCode SDK هو coding/provider bridge، وHermes ACP هو skills/worker bridge المعزول، وDeepSeek Harness يبقى المرشح المختار لـplugin/event spine بعد compatibility gate. أما Electron وProviderGateway وApplication Core وTyped IPC وHuman Gate وAudit وواجهة Osamah فهي طبقة السيطرة الخاصة بالمشروع؛ لا تُعرض OpenCode أو Hermes أو DeepSeek UI، ولا تُشغّل agent loops مكررة لنفس الطلب [1] [2] [3].
