...
import type { MarkdownDestinationWriteResult } from "../application/markdown-destination.js";
...
export interface IpcMethodMap {
  ...
  "brain.memory.searchLocal": { payload: { query: string; limit?: number; visibility?: MemoryVisibility; agentId?: string }; result: readonly MemoryEntry[] };
  ...
  "production.report.markdown.write": { payload: { reportId: string; relativePath: string; approvalId?: string }; result: MarkdownDestinationWriteResult };
...
const isMemorySearchPayload = (value: unknown): boolean => isRecord(value) && hasOnlyKeys(value, ["query", "limit", "visibility", "agentId"]) && isSingleLineString(value.query, 512) && (value.limit === undefined || (typeof value.limit === "number" && Number.isSafeInteger(value.limit) && value.limit >= 1 && value.limit <= 128)) && (value.visibility === undefined || isMemoryVisibilityPayload(value.visibility)) && (value.agentId === undefined || (isString(value.agentId, 128) && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value.agentId)));
...
const isMarkdownDestinationWritePayload = (value: unknown): boolean => isRecord(value)
  && hasOnlyKeys(value, ["reportId", "relativePath", "approvalId"])
  && isString(value.reportId, 256)
  && isString(value.relativePath, 512)
  && isSafeRelativeDestinationPayload(value.relativePath)
  && /\.md$/iu.test(value.relativePath)
  && (value.approvalId === undefined || isString(value.approvalId, 256));
...
const isMethodPayload = (method: string, payload: unknown): boolean => {
  ...
  if (method === "production.report.markdown.write") return isMarkdownDestinationWritePayload(payload);
