...
export const registerEmbeddedSimulatorHandlers = (
  ...
  if (agentDependencies) {
    ...
    transport.register("brain.memory.searchLocal", async (request) => agentDependencies.memoryCapture.searchLocal(request.payload.query, request.payload.limit, { visibility: request.payload.visibility, agentId: request.payload.agentId }));
